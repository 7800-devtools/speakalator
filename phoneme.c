...
int find_rule(char *word, int index, Rule *rules, unsigned char *wordcodes);
...
void xlate_word(char *word, unsigned char *wordcodes)
	...
	do
		...
		index = find_rule(word, index, Rules[type],wordcodes);
		}
	while (word[index] != '\0');
	}
...
*/
		//outstring(output);
		wordcodes[wordcodeindex]=atoi(output);
		wordcodeindex++;
		//printf("%s\n",output);
		return remainder;
		}
...
int leftmatch(char *pattern, char *context)
	//pattern: first char of pattern to match in text
	//context: last char of text to be matched
	{
	char *pat;
	char *text;
	int count;
